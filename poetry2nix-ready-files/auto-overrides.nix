...
final: prev: {
  ...
  orjson = prev.orjson.overridePythonAttrs (old: ((standardMaturin { furtherArgs = { }; }) old));
  ...
  rpds-py = prev.rpds-py.overridePythonAttrs (old: ((standardMaturin { furtherArgs = { }; }) old));
  ...
  rpy2 = prev.rpy2.overridePythonAttrs (
    ...
    lib.optionalAttrs (!(old.src.isWheel or false)) {
      postPatch = lib.optionalString (old.version == "3.4.0") ''
        touch requirements.txt
      '';
      nativeBuildInputs = prev.nativeBuildInputs.nativeBuildInputs or [ ] ++ [ pkgs.R ];
