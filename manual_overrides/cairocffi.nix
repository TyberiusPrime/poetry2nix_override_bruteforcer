  ...
  let
    patch_path =
      ...
      then ./patches/cairocffi/1.7.0
        ...
        (
          if (lib.versionAtLeast old.version "1.1.0")
          then ./patches/cairocffi/1.6.2
          else
            (
              if (lib.versionAtLeast old.version "1.0.0")
              then ./patches/cairocffi/1.0.0
              else patches/cairocffi/0.9
            )
        );
    patches = with pkgs;
      [
